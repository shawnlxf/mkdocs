...
* this
    ...
    ??? example "xxx 会被**自动转化成对象**（JS的糟粕）"

        ![](../img/FF/14-封装成对象.png)

    ??? example "加 'use strict' 解决问题"
    ...
    !!! question "函数如何获取对象的引用(不通过变量名)"
        ...
        ??? note "2. JS 用 this 获取对象"
            ...
            * 两种调用
            ...
            ??? example "例1. 第一个参数要作为 this，但是代码里没有用 this，所以只能用 undefined 占位（null也行）"
